...
@section('content-wrapper')
    <div class="content full-page">
        <form method="POST" action="{{ route('sendcloud.admin.sales.shipments.store', $order->id) }}" @submit.prevent="onSubmit">
            @csrf()
            ...
            <?php
                $isSendcloud = explode('_', $order->shipping_method);
                $isSendcloudMethod = $isSendcloud[0];

                $isAllowApiShippment = core()->getConfigData('sales.carriers.sendcloud.shipment_type');

                if($isSendcloudMethod == 'sendcloud' && $isAllowApiShippment) {
                    $sendcloud = true;
                } else {
                    $sendcloud = false;
                }
                            ...
                            <div class="sale-section">
                                ...
                                <div class="section-content">
                                    ...
                                    @if (! $sendcloud)
                                        ...
                                        <input type="hidden" name="issendcloud" value='0'>
                                    @else
                                        <input type="hidden" class="control" id="shipment[carrier_title]" name="shipment[carrier_title]"/>

                                        <input type="hidden" class="control" id="shipment[track_number]" name="shipment[track_number]"/>
                                    @endif
                                </div>
                            </div>
                        </div>
