...
@section('page-detail-wrapper')
    ...
    <form method="post" action="{{ route('customer.address.edit', $address->id) }}" @submit.prevent="onSubmit">
        ...
        <div class="account-table-content">
            ...
            <?php
                $addresses = explode(PHP_EOL, (old('address1') ?? $address->address1));
                $addresses2 = explode(PHP_EOL, (old('address2') ?? $address->address2));
            ?>
            ...
            <div class="control-group" :class="[errors.has('address2') ? 'has-error' : '']">
                <label for="address2" class="mandatory">{{ __('House Number') }}</label>
                <input type="text" class="control" name="address2" value="{{ isset($addresses2[0]) ? $addresses2[0] : '' }}" id="address_0" v-validate="'required'" data-vv-as="&quot;{{ __('House Number') }}&quot;">
