            ...
            <form id="customer-address-form" method="post" action="{{ route('customer.address.edit', $address->id) }}" @submit.prevent="onSubmit">
                ...
                <div class="account-table-content">
                    ...
                    <?php
                        ...
                        $addresses2 = explode(PHP_EOL, $address->address2);
                    ?>

                    <div class="control-group" :class="[errors.has('address2') ? 'has-error' : '']">
                        <label for="address_0" class="required">{{ __('House Number') }}</label>
                        <input type="text" class="control" name="address2" id="address_0" v-validate="'required'" value="{{ isset($addresses2[0]) ? $addresses2[0] : '' }}" data-vv-as="&quot;{{ __('House Number') }}&quot;">
